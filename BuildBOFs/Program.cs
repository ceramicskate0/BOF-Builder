...
namespace BuildBOFs
{
    class Program
    {
        ...
        private static List<string> CPPFiles = new List<string>();
        ...
        public static void Main(string[] args)
        {
            ...
            Console.WriteLine("[*] Total files Found: " + (CFiles.Count + MakeFIles.Count + msvcFiles.Count + MiscFiles.Count + CPPFiles.Count));
            Console.WriteLine("------------------------------------------------");
            Console.WriteLine("[*] Building .c files");
            foreach (string file in CFiles)
            {
                Console.WriteLine("[*] Working file (" + counter + "\\" + (CFiles.Count + MakeFIles.Count + msvcFiles.Count + MiscFiles.Count + CPPFiles.Count) + ") "+file);
            ...
            Console.WriteLine("------------------------------------------------");
            Console.WriteLine("[*] Building CPP files");
...
");
                ...
                foreach (string d in Directory.GetDirectories(sDir))
                {
                    try
                    {
                        foreach (string f in Directory.GetFiles(d))
                        {
                            ...
                            else if (Path.GetFileName(f).ToLower().Contains(".cpp"))
                            {
                                Console.WriteLine(" [+] Found cpp file: " + f);
        ...
        public static void BuildCPPFile(string CppFIle)
        {
            string filename = Path.GetFileName(CppFIle);
            string ext = Path.GetExtension(CppFIle);
            filename = filename.Replace(ext, "");
            string workingdir = Path.GetDirectoryName(CppFIle);
            ...
            using (var process = Process.Start(new ProcessStartInfo { FileName = @"cmd", Arguments = "", WorkingDirectory = workingdir, UseShellExecute = false, RedirectStandardInput = true, RedirectStandardOutput = true }))
            {
                process.OutputDataReceived += new DataReceivedEventHandler(outdata);
                process.BeginOutputReadLine();
                process.StandardInput.WriteLine(@"cl /EHsc "+ filename + ".cpp");
                process.WaitForExit(timeout);
                //process.StandardInput.WriteLine("exit");
            }
        }
